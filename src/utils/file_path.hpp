...
namespace RT_ISICG
{
	class FilePath
	{
	  public:
		...
		{
			if ( m_filePath.empty() ) { return fp; }
			else if ( fp.empty() )
			{
				return *this;
			}
			FilePath res( *this );
			if ( fp.m_filePath.front() != PATH_SEPARATOR ) { res.m_filePath += PATH_SEPARATOR; }
	  ...
	  private:
		void format()
		{
			...
			{
				if ( m_filePath[ i ] == '\\' || m_filePath[ i ] == '/' ) { m_filePath[ i ] = PATH_SEPARATOR; }
			}
